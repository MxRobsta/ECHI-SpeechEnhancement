...
from shared.signal_utils import noisy_reference_channel
...
class SwishTFGridNet(nn.Module):
    """Online TFGridNetV3.
    ...
    def __init__(
        ...
        self.mask = mask
        self.complex_out = 2 - int(self.mask)
        self.ref_channels = ref_channels
        if self.mask:
            if self.ref_channels is None:
                raise ValueError(
                    f"Ref channel must be provided for masking. {self.ref_channels} not recognised"
                )
            elif not isinstance(self.ref_channels, int):
                self.ref_channels = tuple(self.ref_channels)
        ...
        self.spk_prep = SpeakerPrep(self.spk_dim)

        self.fusions = nn.ModuleList([])
        for _ in range(n_layers):
            if self.in_srcs > 0:
                self.fusions.append(BeefyFiLM(emb_dim, spk_dim))
            else:
                self.fusions.append(Passthrough())
        ...
        self.deconv = nn.ConvTranspose2d(
            emb_dim, out_srcs * (2 - int(self.mask)), ks, padding=padding
        )

    def get_test_sample(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:

        return (
            torch.rand([1, self.n_imics, 125, 65, 2]),
            torch.rand([1, self.in_srcs, self.spk_dim]),
            torch.tensor([]),
        )
    ...
    def forward(
        ...
        Args:
            spec: Noisy spectrogram - [batch, n_imics, frames, freqs, 2]
            spkid: Speaker id audio - [batch, in_srcs, spk_dim]
            spk_lens: Lenths of spkid audio - [batch, 1]

        Returns:
            enhanced: Spectrogram of shape [batch, out_srcs, frames, freqs, 2]
            spk_pred: probability of speaker being present - [batch]
        """
        ...
        if self.mask:
            spec = torch.complex(spec[..., 0], spec[..., 1])
            spec = noisy_reference_channel(spec, self.ref_channels)  # type: ignore
            batch = batch * spec
        else:
            batch = batch.view([n_batch, self.out_srcs, 2, n_frames, n_freqs])
            batch = torch.complex(batch[:, :, 0], batch[:, :, 1])

        return batch, None  # , ilens, OrderedDict()
...
class SwishActivation(nn.Module):
    def __init__(self):
        ...
        self.beta = nn.Parameter(torch.Tensor(1).to(torch.float32))

    def forward(self, x):
        return x * self.sigmoid(self.beta * x)


class SpeakerPrep(nn.Module):
    def __init__(self, spk_dim):
        super().__init__()
        self.project = nn.Linear(spk_dim, 2 * spk_dim)
        self.swish = SwishActivation()
        self.reduce = nn.Linear(2 * spk_dim, spk_dim)

    def forward(self, x):
        """
        Assumes shape [batch, n_speakers, spk_dim]
        Returns [batch, spk_dim]
        """
        x = self.swish(self.project(x))
...
class FiLM(nn.Module):
    ...
    def forward(self, cond, x):
        ...
        cond: [B, cond_dim]
        """
        gamma = self.gamma_fc(cond).unsqueeze(1).unsqueeze(1)  # [B, 1, 1, C]
        beta = self.beta_fc(cond).unsqueeze(1).unsqueeze(1)  # [B, 1, 1, C]
        return gamma * x + beta


class BeefyFiLM(nn.Module):
    def __init__(self, feature_dim, cond_dim):
        super(BeefyFiLM, self).__init__()
        self.project = nn.Linear(feature_dim, feature_dim)
        self.swish = SwishActivation()
    ...
    def forward(self, condition, x):
        ...
        batch = self.swish(self.project(batch))
        batch = self.film(condition, batch)
...
if __name__ == "__main__":
    model = SwishTFGridNet(n_imics=4, in_srcs=1, out_srcs=1)
    audio = torch.rand(1, 4, 1000, 65, 2)
    aux = torch.rand(1, 1, 256)
    aux_lens = torch.tensor([])
    out, out2 = model(audio, aux, aux_lens)
    print(out.shape)
