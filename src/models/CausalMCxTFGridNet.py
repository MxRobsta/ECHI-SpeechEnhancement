...
class MCxTFGridNet(nn.Module):
    ...
    def __init__(
        ...
        self.aux_enc = AuxEncoder(emb_dim * self.in_srcs, in_srcs)
        self.sigmoid = torch.nn.Sigmoid()

        self.fusions = nn.ModuleList([])
        for _ in range(n_layers):
            if self.in_srcs > 0:
                self.fusions.append(FiLM(emb_dim, emb_dim * self.in_srcs))
            else:
        ...
        self.deconv = nn.ConvTranspose2d(emb_dim, out_srcs * 2, ks, padding=padding)

    def get_test_sample(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:

        return (
            torch.rand([1, self.n_imics, 125, 65, 2]),
            torch.rand([1, self.in_srcs, 125 * 30, 65, 2]),
            torch.tensor([[125 * 30 for _ in range(self.in_srcs)]]),
        )
    ...
    def forward(
        ...
        Args:
            ...
            spkid: Speaker id audio - [batch, in_srcs, frames, freqs, 2]
        ...
        """
        # assert torch.is_complex(spec) and torch.is_complex(spk), f"Requires complex inputs:\n\tSpec: {spec.dtype}\n\tSpk: {spk.dtype}"
        ...
        if self.in_srcs > 0:
            ...
            spk_feat = spk.moveaxis(-1, 2)  # [batch, srcs, 2, frames, freqs]
            if self.n_imics > 1:
                spk_feat = spk_feat.repeat(1, 1, self.n_imics, 1, 1)

            new_spk_feat = []
            for i in range(spk_feat.shape[1]):
                new_spk_feat.append(self.conv(spk_feat[:, i]))

            spk_feat = torch.stack(new_spk_feat, dim=1)
            _, _, _, T, F = spk_feat.shape
            spk_feat = spk_feat.reshape(n_batch, self.in_srcs * self.emb_dim, T, F)

            spk_feat, spk_pred = self.aux_enc(spk_feat, spk_lens.squeeze(0))
            spk_pred = self.sigmoid(spk_pred)
        else:
            spk_pred = torch.tensor([[]])
        ...
        batch = batch.moveaxis(2, -1)

        return batch, spk_pred.squeeze(1)  # , ilens, OrderedDict()
...
if __name__ == "__main__":
    model = MCxTFGridNet(n_imics=4, in_srcs=1, out_srcs=1)
    audio = torch.rand(1, 4, 1000, 65, 2)
    aux = torch.rand(1, 1, 996, 65, 2)
    aux_lens = torch.tensor([[996]])
