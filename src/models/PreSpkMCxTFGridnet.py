...
class PreSpkMCxTFGridNet(nn.Module):
    """Online TFGridNetV3.
    ...
    def __init__(
        ...
        for _ in range(n_layers):
            if self.in_srcs > 0:
                self.fusions.append(FiLM(emb_dim, spk_dim * self.in_srcs))
            else:
    ...
    def forward(
        ...
        if self.in_srcs == 0:
            spk_feat = torch.tensor([], requires_grad=True)
        elif self.in_srcs == 1:
            spk_feat = spk.squeeze(1)
        else:
            spk_feat = spk.reshape(n_batch, self.in_srcs * self.spk_dim)
...
if __name__ == "__main__":
    model = PreSpkMCxTFGridNet(n_imics=4, in_srcs=1, out_srcs=1)
